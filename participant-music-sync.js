(() => {
  ...
  function replayCurrentPlayback(playback) {
    ...
    const key = `${room()}|${command.senderId || ""}|${command.playing ? "1" : "0"}|${Math.round(Number(command.positionMs || 0) / 1000)}|${command.track?.isrc || spotifyUriFrom(command)}`;
  ...
  async function handleRemoteState(command) {
    ...
    try {
      ...
      const position = command.playing ? commandPosition(command) : Math.max(0, Number(command.positionMs) || 0);

...
class GameClient {
  ...
  async nextEnded() {
    const packet = await this.waitFor((message) => message.startsWith('42["game:ended"'));
    return JSON.parse(packet.slice(2))[1];
  }
...
test("tom aktiv tidslinje viser kun årtier, og challenge-valget vises først efter lås", () => {
  ...
  game.players[0].selectedDecade = 1980;
  ...
  assert.match(challengerHtml, /1980'erne · Michael/);
  assert.match(challengerHtml, /data-decade="1980" disabled/);
});
...
test("flere challengers svarer i trykrækkefølge og får individuelle tidslinjer", async () => {
  ...
  const activeDecadeTaken = await clients[2].emitWithAck("song:decade", { code, decade: 1980 });
  assert.equal(activeDecadeTaken.ok, false);
  await selectAndLock(clients, 2, "song:decade", { code, decade: 1970 });
  const firstChallengerDecadeTaken = await clients[1].emitWithAck("song:decade", { code, decade: 1970 });
  assert.equal(firstChallengerDecadeTaken.ok, false);
...
test("challengers bruger den aktive tidslinje og kan ikke genbruge optagede placeringer", async () => {
  const host = new GameClient();
  await host.connect();
  const created = await host.emitWithAck("game:create", { playerName: "Aktiv", playerId: "shared-host" });
  const code = created.code;
  await host.nextGame();

  await start([host], code, 2000, "Første");
  await selectAndLock([host], 0, "song:decade", { code, decade: 2000 });
  await finishAndNext([host], code);
  await start([host], code, 2010, "Anden");
  await selectAndLock([host], 0, "song:place", { code, slot: 1 });
  await finishAndNext([host], code);

  const first = new GameClient();
  const second = new GameClient();
  await Promise.all([first.connect(), second.connect()]);
  await first.emitWithAck("game:join", { code, playerName: "Peter", playerId: "shared-first" });
  await Promise.all([host.nextGame(), first.nextGame()]);
  await second.emitWithAck("game:join", { code, playerName: "Sofie", playerId: "shared-second" });
  const clients = [host, first, second];
  await Promise.all(clients.map((client) => client.nextGame()));

  await start(clients, code, 2005, "Mellem");
  await selectAndLock(clients, 0, "song:place", { code, slot: 2 });
  await event(clients, 1, "song:challenge", code);
  await event(clients, 2, "song:challenge", code);

  const activeSlotTaken = await first.emitWithAck("song:place", { code, slot: 2 });
  assert.equal(activeSlotTaken.ok, false);
  const firstLocked = await selectAndLock(clients, 1, "song:place", { code, slot: 1 });
  assert.deepEqual(firstLocked.game.players.find((player) => player.id === "shared-host").timeline, [2000, 2010]);

  const challengerSlotTaken = await second.emitWithAck("song:place", { code, slot: 1 });
  assert.equal(challengerSlotTaken.ok, false);
  await selectAndLock(clients, 2, "song:place", { code, slot: 0 });
  const revealed = await event(clients, 0, "song:reveal", code);
  assert.deepEqual(revealed.game.players.find((player) => player.id === "shared-host").timeline, [2000, 2010]);
  assert.deepEqual(revealed.game.players.find((player) => player.id === "shared-first").timeline, [2005]);
  assert.deepEqual(revealed.game.players.find((player) => player.id === "shared-second").timeline, []);
  assert.equal(revealed.game.players.find((player) => player.id === "shared-first").score, 1);
  clients.forEach((client) => client.close());
});

test("ingen får point eller sang, når alle valgte årtier er forkerte", async () => {
  const { clients, code } = await setupPlayers(["none-host", "none-guest"]);
  await start(clients, code, 2005);
  await selectAndLock(clients, 0, "song:decade", { code, decade: 1980 });
  await event(clients, 1, "song:challenge", code);
  await selectAndLock(clients, 1, "song:decade", { code, decade: 1970 });
  const revealed = await event(clients, 0, "song:reveal", code);
  revealed.game.players.forEach((player) => {
    assert.equal(player.score, 0);
    assert.deepEqual(player.timeline, []);
  });
  clients.forEach((client) => client.close());
});
...
test("reconnect bevarer challenge-kø og låste svar midt i flowet", async () => {
  ...
  const blocked = await clients[2].emitWithAck("song:decade", { code, decade: 1980 });
  assert.equal(blocked.ok, false);
  await selectAndLock([clients[0], reloaded, clients[2]], 2, "song:decade", { code, decade: 1970 });
...
test("kun værten kan starte forfra, og alle klienter nulstilles", async () => {
  const { clients, code } = await setupPlayers(["reset-host", "reset-guest"]);
  await start(clients, code, 1976);
  await selectAndLock(clients, 0, "song:decade", { code, decade: 1970 });
  await event(clients, 1, "song:challenge", code);
  assert.equal((await clients[1].emitWithAck("game:restart", code)).ok, false);
  const reset = await clients[0].emitWithAck("game:restart", code);
  const states = await Promise.all(clients.map((client) => client.nextGame()));
  assert.equal(reset.ok, true);
  states.forEach((state) => {
    assert.equal(state.code, code); assert.equal(state.phase, "lobby"); assert.equal(state.currentSong, null);
    assert.deepEqual(state.players.map((player) => player.name), ["reset-host", "reset-guest"]);
    state.players.forEach((player) => { assert.equal(player.score, 0); assert.deepEqual(player.timeline, []); assert.equal(player.challengesRemaining, 5); });
  });
  assert.equal((await start(clients, code, 2001)).ok, true);
  clients.forEach((client) => client.close());
});

test("reconnect virker efter start forfra", async () => {
  const { clients, code } = await setupPlayers(["restart-host", "restart-guest"]);
  await event(clients, 0, "game:restart", code);
  await clients[1].disconnect(); await clients[0].nextGame();
  const resumed = new GameClient(); await resumed.connect();
  const result = await resumed.emitWithAck("game:resume", { code, playerId: "restart-guest" });
  assert.equal(result.ok, true); assert.equal(result.game.players[1].challengesRemaining, 5);
  clients[0].close(); resumed.close();
});

test("deltager kan forlade permanent og joine igen som ny", async () => {
  const { clients, code } = await setupPlayers(["leave-host", "leave-guest"]);
  assert.equal((await clients[1].emitWithAck("game:leave", code)).ok, true);
  assert.deepEqual((await clients[0].nextGame()).players.map((player) => player.id), ["leave-host"]);
  assert.equal((await clients[1].emitWithAck("game:resume", { code, playerId: "leave-guest" })).ok, false);
  const joined = await clients[1].emitWithAck("game:join", { code, playerName: "Ny", playerId: "leave-guest" });
  assert.equal(joined.ok, true); assert.equal(joined.game.players[1].name, "Ny");
  clients.forEach((client) => client.close());
});

test("challenge-valg frigives og køen fortsætter når en challenger forlader", async () => {
  const { clients, code } = await setupPlayers(["flow-host", "flow-one", "flow-two"]);
  await start(clients, code, 1984); await selectAndLock(clients, 0, "song:decade", { code, decade: 1990 });
  await event(clients, 1, "song:challenge", code); await event(clients, 2, "song:challenge", code);
  await selectAndLock(clients, 1, "song:decade", { code, decade: 1970 });
  assert.equal((await clients[1].emitWithAck("game:leave", code)).ok, true);
  const state = await clients[0].nextGame(); await clients[2].nextGame();
  assert.equal(state.challengeQueue[state.challengeTurnIndex], "flow-two");
  assert.equal(state.players.some((player) => player.selectedDecade === 1970), false);
  const remaining = [clients[0], clients[2]];
  assert.equal((await event(remaining, 1, "song:decade", { code, decade: 1970 })).ok, true);
  await event(remaining, 1, "song:lock", code); assert.equal((await event(remaining, 0, "song:reveal", code)).ok, true);
  clients.forEach((client) => client.close());
});

test("aktiv deltager der forlader giver turen videre", async () => {
  const { clients, code } = await setupPlayers(["turn-host", "turn-active", "turn-next"]);
  await start(clients, code, 1975); await selectAndLock(clients, 0, "song:decade", { code, decade: 1970 });
  await event(clients, 1, "song:pass", code); await event(clients, 2, "song:pass", code); await finishAndNext(clients, code);
  await start(clients, code, 1985); assert.equal((await clients[1].emitWithAck("game:leave", code)).ok, true);
  const state = await clients[0].nextGame(); await clients[2].nextGame();
  assert.equal(state.phase, "lobby"); assert.equal(state.currentSong, null); assert.equal(state.activePlayerId, "turn-next");
  clients.forEach((client) => client.close());
});

test("kun værten kan afslutte og gammel kode eller session kan ikke genbruges", async () => {
  const { clients, code } = await setupPlayers(["end-host", "end-guest"]);
  assert.equal((await clients[1].emitWithAck("game:end", code)).ok, false);
  const notices = clients.map((client) => client.nextEnded());
  assert.equal((await clients[0].emitWithAck("game:end", code)).ok, true);
  (await Promise.all(notices)).forEach((notice) => assert.match(notice.message, /afsluttet/));
  assert.equal((await clients[1].emitWithAck("game:resume", { code, playerId: "end-guest" })).ok, false);
  assert.equal((await clients[1].emitWithAck("game:join", { code, playerName: "Ny", playerId: "new" })).ok, false);
  clients.forEach((client) => client.close());
});

test("spilstyring viser handlinger efter rolle", () => {
  const game = { code: "TOOLS", hostId: "host", activePlayerId: "host", roundPlayerId: null, phase: "lobby", roundNumber: 0,
    challengeQueue: [], challengeTurnIndex: 0, challengeEligible: [], challengeDecisions: {}, showAnswer: false, currentSong: null,
    players: [{ id: "host", name: "Host", connected: true, score: 0, timeline: [], challengesRemaining: 5 }, { id: "guest", name: "Guest", connected: true, score: 0, timeline: [], challengesRemaining: 5 }] };
  const host = renderClient(game, "host"); const guest = renderClient(game, "guest");
  assert.match(host, /restartGame/); assert.match(host, /endGame/); assert.doesNotMatch(host, /leaveGame/);
  assert.match(guest, /leaveGame/); assert.doesNotMatch(guest, /restartGame|endGame/);
});
